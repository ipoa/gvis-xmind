(function () {
  ...
  GraphVisXmind.prototype.buildXmindNode = function (rootNode) {
    ...
    this.buildVisTreeNode(
      { children: subTree.rightNodes },
      rootVisNode,
      'right'
    );
    this.layout(
      rootVisNode,
      this.option.levelSpace,
      this.option.nodeSpace,
      'right'
    );
  ...
  GraphVisXmind.prototype.clickNode = function (node) {
    ...
    if (node.selCtrlNode && node.hasChild) {
      if ((node.outLinks || []).length == 0) {
        ...
        node.expandCtrlNode = false;
      } else {
        self.contractChild(node);
        node.expandCtrlNode = true;
      }
    } else {
  ...
  //查找所有上级节点
  GraphVisXmind.prototype.findAllPreLevelNodes = function (
    node,
    preLevelNodes = []
  ) {
    var self = this;
  ...
  //构建可视化叶子节点
  GraphVisXmind.prototype.buildVisTreeNode = function (
    treeNode,
    parentNode,
    direct = 'right'
  ) {
  ...
  GraphVisXmind.prototype.createVisNode = function (child, parentNode, direct) {
    var self = this;
    var subNode = self.visgraph.addNode({
      id: child.id,
    ...
    // subNode.hasChild =( (child.children || []).length > 0) && subNode.edgeNum > 0;
  ...
  GraphVisXmind.prototype.reloadLayout = function (treeData, isAppend) {
    ...
    if (isAppend) {
      if (self.currentNode.isRootNode) {
        ...
        self.buildVisTreeNode(
          { children: subTree.rightNodes },
          self.currentNode,
          'right'
        );
        self.buildVisTreeNode(
          { children: subTree.leftNodes },
          self.currentNode,
          'left'
        );
      } else {
        self.buildVisTreeNode(
          { children: self.newChildren },
          self.currentNode,
          self.currentNode.directType
        );
      }
  ...
  // 收起不同级别的节点
  GraphVisXmind.prototype.collapseChildByLevel = function (targetLevel) {
    if (targetLevel <= 0) {
      return null;
    }
    var self = this;
    var rootVisNode = self.rootVisNode;
    function collapseChild(node, targetLevel, currentLevel = 0) {
      var subNodes = node.leftSubNodes.concat(node.rightSubNodes);
      if (targetLevel == currentLevel) {
        if (!node.expandCtrlNode && node.hasChild) {
          node.expandCtrlNode = true;
          self.contractChild(node);
        }
      } else if (subNodes.length > 0 && targetLevel > currentLevel) {
        node.leftSubNodes = node.leftSubNodes.map(function (node) {
          return collapseChild(node, targetLevel, currentLevel + 1);
        });
        node.rightSubNodes = node.rightSubNodes.map(function (node) {
          return collapseChild(node, targetLevel, currentLevel + 1);
        });
      }
      return node;
    }
    self.rootVisNode = collapseChild(rootVisNode, targetLevel);
  };

  // 展开不同级别的节点
  GraphVisXmind.prototype.expandChildByLevel = function (targetLevel) {
    if (targetLevel <= 0) {
      return null;
    }
    var self = this;
    var rootVisNode = self.rootVisNode;
    var delay = 0;
    function expandChild(node, targetLevel, currentLevel = 0) {
      var subNodes = node.leftSubNodes.concat(node.rightSubNodes);
      if (targetLevel == currentLevel) {
        if (node.expandCtrlNode && node.hasChild) {
          node.expandCtrlNode = false;
    ...
    expandChild(rootVisNode, targetLevel);
  };
  //构建虚拟的脑图
  ...
  //构建虚拟的叶子节点
  GraphVisXmind.prototype.buildVirtualSubNode = function (
    treeNode,
    parentNode,
    direct = 'right'
  ) {
  ...
  GraphVisXmind.prototype.drawLine = function (rootNode) {
    ...
    subNodes.forEach((node, i) => {
      ...
      if (exsistLink) {
        ...
        var edge = self.visgraph.addEdge({
          source: rootNode.id,
          target: node.id,
        });
        if (edge) {
  ...
  GraphVisXmind.prototype.drawRectNode = function (ctx) {
    ...
    if (this.warnText) {
      ...
      ctx.fillText(
        this.warnText,
        -this.width / 2 + this.nodePadding + 15,
        sigleWarnTextWidth + 10
      );
      ctx.restore();
      ...
      ctx.beginPath();
      ctx.translate(
        -this.width / 2 + this.nodePadding + 5,
        sigleWarnTextWidth + 5
      );
  ...
  //平移节点坐标
  GraphVisXmind.prototype.shiftLeftTreeNode = function (
    subNodes,
    shiftOffsetY
  ) {
    var self = this;
  ...
  //执行布局计算
  GraphVisXmind.prototype.layout = function (
    node,
    BaseOffsetX = 140,
    BaseOffsetY = 10,
    direct = 'right'
    ...
    for (var i = 0; i < subNodeLength; i++) {
      ...
      if (childNode.warnText) {
        childNode.finishY =
          node.finishY + offsetY + self.option.nodePadding / 2;
      } else {
  ...
  GraphVisXmind.prototype.deepExtend = function (
    a,
    b,
    protoExtend,
    allowDeletion
